...
class AutoTrader(BaseAutoTrader):
    ...
    def on_order_book_update_message(
        ...
        if self.bids is not None:
            ...
            elif not self.last_bids[Instrument.ETF][2] and self.last_bids[Instrument.ETF][2] > self.bids.price:
                self.send_cancel_order(self.bid_id)
                self.bid_id = 0
                self.bids = None
        ...
        if self.asks is not None:
            ...
            elif not self.last_asks[Instrument.ETF][2] and self.last_asks[Instrument.ETF][2] < self.asks.price:
                self.send_cancel_order(self.ask_id)
                self.ask_id = 0
                self.asks = None
        ...
        if self.bids is None:
            if self.last_bids[Instrument.FUTURE][0] > self.last_bids[Instrument.ETF][0]:
        ...
        if self.asks is None:
            if self.last_asks[Instrument.FUTURE][0] < self.last_asks[Instrument.ETF][0]:
